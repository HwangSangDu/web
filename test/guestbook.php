...
$connect = mysql_connect("localhost","user","pass") or die("SQL server에 연결할 수 없습니다."); 
      ...
      if (!$start) { $start = 0; }
       ...
       for($i=$start ; $i<$start+$scale ; $i++) {  //  start 에서 scale 까지 만
         ...
         <TD width='109' bgcolor='#5485B6'><P align='center'><FONT face='굴림' size='2' color='#CDDAE4'>
             ...
             <IMG src='./img/del.gif' width='22' height='13' border='0' alt='지우기'></a></FONT></TD>
         ...
         <IMG SRC=./img/mail.gif BORDER=0 ALT='편지쓰기' align=center><FONT face='굴림' size='2'
         color='#E7EEF5'>$row[email]</FONT></A> &nbsp;");
         if ($row[home]) echo("<a href=http://$row[home]><IMG SRC=./img/home.gif BORDER=0 ALT='홈페이지' 
...
if (!$mode || $result) {  // ==================== 하단 버튼 ==============
   ...
   if ($p_p >= 0 && $mode != 'new') {
      echo("<a href='$PHP_SELF?start=$p_p'><IMG HEIGHT=30 WIDTH=30 SRC='./img/p-doc.gif' VSPACE=0
      HSPACE=0 ALIGN='TOP' BORDER=0 alt='이전 페이지'></a> ");
   }

   if ($n_p < $total && $mode != 'new') {
      echo("<a href='$PHP_SELF?start=$n_p'><IMG HEIGHT=30 WIDTH=30 SRC='./img/n-doc.gif' VSPACE=0
